...
const Timer = () => {
  ...
  return (
    <>
     {showConfirmation && (
      <div className="modal-overlay" style={{ position: 'fixed', top: '-133', left: '92', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="confirmation-modal" style={{ zIndex: '100', backgroundColor: 'rgba(0, 0, 0, 0.5)', padding: '20px', borderRadius: '8px' }}>
        <Dialog
      ...
      >
        ...
        {`Are you sure you want to ${confirmationAction === "checkout" ? "check out" : "check in"}?`}
        </DialogTitle>
    ...
    <div className="card mb-3 rounded-3 shadow shadow-lg" style={{ maxWidth: "400px", maxHeight: "392px" }}>
      <ToastContainer />
      ...
      <div className="row g-0">
        ...
        <h1 className="text-center pt-3">
          {isCheckInRunning ? formatTime(checkInElapsedTime) : formatTime(breakElapsedTime)}Hrs
        </h1>

        <div style={{ fontSize: "12px", color: '#a9a7a7', backgroundColor: '#f5f1f1', borderRadius: '4px', fontWeight: '500', width: '82%', padding: '18px 25px 0px 25px', marginTop: '7px', marginLeft: '33px', marginBottom: '27px' }}>
        ...
        <div className="d-flex   mb-5 mt-3" style={{ paddingLeft: '29px' }}>
          <button onClick={handleCheckButtonClick} className={isCheckInRunning ? "checkout-button" : "checkin-button"} style={{ width: '155px', marginRight: '13px' }}>
            {isCheckInRunning ? "Check Out" : "Check In"}
          </button>
          <button onClick={handleBreakButtonClick} className={isBreakStartRunning ? "breakend-button" : "breakstart-button"} style={{ width: '155px' }} disabled={!isCheckInRunning}>
